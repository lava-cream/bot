...
import { ButtonInteraction, Collection, Snowflake } from 'discord.js';
...
export default class SpamCommand extends Command {
	public override async chatInputRun(command: Command.ChatInputCommandInteraction<'cached'>) {
		...
		if (gatheredPlayers.isErr())
			return send(command, (builder) => builder.addEmbed(() => EmbedTemplates.createSimple(gatheredPlayers.unwrapErr())));
		...
		await Promise.allSettled(
			[...this.splitPrice(prize, winners).values()]
			...
			builder
				.addEmbed(() =>
					EmbedTemplates.createSimple(
						join(
							[...winners.concat(losers).values()].map(
								({ button, spams, won }, idx, arr) =>
									`${spams > 0 ? ['🥇', '🥈', '🥉'].at(idx) ?? '👏' : '💀'} ${inlineCode(
										toInlineNumberCode(
											arr.map((n) => n.spams),
											idx,
											InlineNumberCodeAlignment.Right
										)
									)} - ${bold(button.member.nickname ?? button.user.username)} won ${
										spams > 0 ? `${bold(won.toLocaleString())} coins` : 'nothing'
									}!`
							)
						)
			...
			.addEmbed(() =>
				EmbedTemplates.createCamouflaged()
					...
					.setFields({
						...
						value: players.size ? players.map(({ button }) => userMention(button.member.user.id)).join(', ') : 'No players yet.'
					})
			)
			.addRow((row) =>
						...
						actions: {
							[customId.create(Control.Join)]: async (ctx) => {
								if (players.has(ctx.interaction.user.id)) {
									await send(ctx.interaction, (builder) =>
										builder.addEmbed(() => EmbedTemplates.createSimple('You already joined!'))
									);
									return;
								}
								...
								await send(ctx.interaction, (builder) =>
									builder.addEmbed(() => EmbedTemplates.createSimple(`You successfully joined the event!`))
								);
							...
							[customId.create(Control.Start)]: async (ctx) => {
								if (ctx.interaction.user.id !== command.user.id) {
									await send(ctx.interaction, (builder) =>
										builder.addEmbed(() => EmbedTemplates.createSimple("Hey! You're not the event host!"))
									);
									return;
								}

								await send(ctx.interaction, (builder) =>
									builder.addEmbed(() => EmbedTemplates.createSimple('You started the event!'))
								);
								return ctx.stop();
							},
							[customId.create(Control.Stop)]: async (ctx) => {
								...
								await send(ctx.interaction, (builder) =>
									builder.addEmbed(() => EmbedTemplates.createSimple('You stopped the event!'))
								);
								return ctx.stop();
							}
						},
		...
		return Result.fromAsync(
			() =>
				new Promise(async (resolve, reject) => {
					...
					const message = await payload.command.fetchReply(),
						{ thread } = message;
					...
					collector.once('end', async () => {
						if (mode === Mode.Button)
							await sentMessage.inspectAsync((message) => message.edit({ components: disableMessageComponents(message.components) }));

						await thread.send(
							new MessageContentBuilder().addEmbed(() => EmbedTemplates.createSimple('The results are in and the event has ended!'))
						);
						await thread.edit({ locked: true, archived: true, reason: `${mode} spam heist ended` }).catch(noop);
					...
					await send(command, (builder) =>
						builder
							...
							.addRow((row) =>
								row.addButtonComponent((btn) => btn.setStyle(ButtonStyle.Link).setLabel(thread.name).setURL(sentMessage.unwrap().url))
							)
					);
				})
		);
	...
	private splitPrice(prize: number, players: SpamCommand.Players): SpamCommand.Players {
		...
		return players.sort((a, b) => b.spams - a.spams).each((player) => Reflect.set(player, 'won', Math.trunc(spamWorth * player.spams)));
	}
		...
		return Result.fromAsync(
			() =>
				new Promise(async (resolve, reject) => {
					...
					if (commandMessage.channel.type !== ChannelType.GuildText) return reject('Not in a text channel.');

					const atEveryoneRole = commandMessage.channel.permissionOverwrites.resolve(payload.command.guildId);
					if (!isNullOrUndefined(atEveryoneRole)) await atEveryoneRole.edit({ SendMessagesInThreads: payload.mode === Mode.Message });
					...
					const content = new MessageContentBuilder().addEmbed(() =>
						EmbedTemplates.createSimple(
							`Spam ${bold(commandMessage.guild.name)} ${inlineCode(
								Math.trunc(Config.MaxSpamAmount * payload.players.size).toLocaleString()
							)} times!`
						)
					);
		...
		switch (payload.mode) {
			...
			case Mode.Message: {
				return payload.thread.createMessageCollector({
					...
					time: minutes(payload.command.options.getNumber('minutes') ?? 1),
					filter: (message) =>
						payload.players.has(message.author.id) && message.content.toLowerCase().includes(payload.command.guild.name.toLowerCase())
				});
			}
		}
	}
			...
			(builder) =>
				builder
					...
					.addNumberOption((option) =>
						option.setName('prize').setDescription('The prize pool to be split by the players.').setRequired(true).setMinValue(3_000_000)
					)
					...
					.addNumberOption((option) =>
						option.setName('minutes').setDescription('The duration of the spam, in minutes.').setMinValue(1).setMaxValue(5)
					),
			{
