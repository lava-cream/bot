...
export default class ShareCommand extends Command {
	public override async chatInputRun(command: Command.ChatInputCommandInteraction<'cached'>) {
		...
		const user = command.options.getMember('user');
		const amount = command.options.getString('amount', true);

		if (isNullOrUndefined(user)) return ResponderError.send(command, 'That member is not in this server.');
		...
		if (parsedAmount > db.wallet.value)
			return ResponderError.send(command, `You can't share that many. You only have ${bold(db.wallet.value.toLocaleString())} coins!`);
		...
		await send(
			command,
			ShareCommand.renderSuccessfulTransactionMessage(parsedAmount, { db, user: command.user }, { db: dbRecepient, user: user.user })
		);
		return;
	}
