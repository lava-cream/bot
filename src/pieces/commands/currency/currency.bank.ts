...
export default class BankCommand extends Subcommand {
	public async chatInputDeposit(command: Subcommand.ChatInputCommandInteraction<'cached'>) {
		...
		return send(command, (builder) =>
			builder.addEmbed(() =>
				EmbedTemplates.createCamouflaged().addFields(
					{
						name: 'Deposited',
	...
	public async chatInputWithdraw(command: Subcommand.ChatInputCommandInteraction<'cached'>) {
		const db = await this.container.db.players.fetch(command.user.id);
	...
	public async chatInputSpace(command: Subcommand.ChatInputCommandInteraction<'cached'>) {
		const db = await this.container.db.players.fetch(command.user.id);
				...
				[customId.create('set-multiplier')]: async (ctx) => {
					...
					await ctx.interaction.showModal(
						new ModalBuilder()
							...
							.setComponents(
								new ActionRowBuilder<TextInputComponentBuilder>().addTextInputComponent((input) =>
									input
					...
					await edit(modal, (builder) =>
						builder.addEmbed(() => EmbedTemplates.createSimple(`Successfully set your bank space multiplier to ${parsedInput}%.`))
					);
					return ctx.stop();
				}
			},
		...
		return new InteractionMessageContentBuilder()
			...
			.addRow((row) =>
				row.addButtonComponent((btn) =>
					btn.setLabel('Set Multiplier').setCustomId(customId.create('set-multiplier')).setStyle(ButtonStyle.Secondary).setDisabled(ended)
				)
			);
	}
					...
					.addSubcommand((subcommand) =>
						subcommand
							...
							.addStringOption((option) =>
								option.setName('amount').setDescription('The amount you want to withdraw.').setRequired(true)
							)
					)
					.addSubcommand((subcommand) => subcommand.setName('space').setDescription('Information regarding your bank space.')),
			{
