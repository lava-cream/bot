...
import { Collector, MessageContentBuilder, InteractionMessageContentBuilder, CustomId, edit, unsend, send, EmbedTemplates } from '#lib/utilities';
...
export default class EvalCommand extends Command {
	...
	public override async chatInputRun(command: Command.ChatInputCommandInteraction<'cached'>) {
		...
		await send(command, (content) => content.addEmbed((embed) => embed.setDescription('Please wait...').setColor(Colors.DarkButNotBlack)));
		context.watch.start();
		...
		const renderEvaluatedCodeMessage = () => {
			return new MessageContentBuilder().addEmbed(() => EmbedTemplates.createSimple(codeBlock('js', code)).setTitle('Evaluated Code'));
		};
