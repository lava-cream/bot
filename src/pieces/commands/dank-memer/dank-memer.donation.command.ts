...
export default class DonationCommand extends Subcommand {
  ...
  public async chatInputCreateCategory(command: CommandInteraction<'cached'>) {
    ...
    await db
      .run((db) => db.categories.create({ id, name, multiplier, logs: logs?.id, default: false, status: DonationTrackerCategoryStatus.Enabled }))
      .save();
  ...
  public async chatInputDeleteAutorole(command: CommandInteraction<'cached'>) {
    ...
    const resolvedAutorole = db.autoroles.resolve(role.id);
    if (isNullOrUndefined(resolvedAutorole))
      throw new CommandOptionError({ message: 'The input did not resolve to an existing autorole.', option: 'role' });
  ...
  public async chatInputWeeklyLeaderboard(command: CommandInteraction<'cached'>) {
    ...
    if (isNullOrUndefined(resolvedCategory))
      throw new CommandOptionError({ message: 'The input did not resolve to an existing donation category.', option: 'category' });

    await paginate(
      command,
      chunk(
        resolvedCategory.donators.entries
        ...
        new MessageContentBuilder().addEmbed((embed) =>
          embed
  ...
  public async chatInputWeeklyReset(command: CommandInteraction<'cached'>) {
    ...
    if (isNullOrUndefined(defaultCategory))
      throw new CommandError({ message: 'This server has not configured a default donation category. Please ask the server staff to do so.' });
  ...
  public async chatInputLogsSet(command: CommandInteraction<'cached'>) {
    ...
    const resolvedCategory = DonationCommand.resolveCategory(category, db);
    if (isNullOrUndefined(resolvedCategory))
      throw new CommandOptionError({ message: 'The input did not resolve into a valid donation category.', option: 'category' });
  ...
  public async chatInputAdd(command: CommandInteraction<'cached'>) {
    ...
    if (isNullOrUndefined(resolvedCategory)) {
      await edit(command, (builder) =>
        builder.addEmbed((embed) =>
          embed.setTitle('Category Not Found').setColor(Constants.Colors.RED).setDescription('Cannot find that donation category.')
        )
      );
      return;
    }
    ...
    await this.container.db.trackers.updateDonator(db, {
      amount: {
        amount,
  ...
  public async chatInputProfile(command: CommandInteraction<'cached'>) {
    ...
    const resolvedCategory = DonationCommand.resolveCategory(category, db);
    if (isNullOrUndefined(resolvedCategory))
      throw new CommandOptionError({ message: 'The input did not resolve into an existing category.', option: 'category' });
    ...
    const renderContent = (category: DonationTrackerCategorySchema, ended: boolean) => {
      ...
      return new InteractionMessageContentBuilder()
        .addEmbed((embed) =>
          embed.setTitle(`${member.user.username}'s donation profile`).setColor(randomColor()).setFields(
            {
  ...
  public async chatInputLeaderboard(command: CommandInteraction<'cached'>) {
    ...
    const resolvedCategory = DonationCommand.resolveCategory(category, db);
    if (isNullOrUndefined(resolvedCategory))
      throw new CommandOptionError({ message: 'The input did not resolve into an existing donation category.', option: 'category' });
    ...
    await edit(command, (builder) =>
      builder.addEmbed((embed) =>
        embed.setTitle(`Top ${top10.length} "${resolvedCategory.name}" Donators`).setDescription(
          join(
            ...top10.map((donor, idx) => {
  ...
  public static resolveCategory(query: string, db: DonationTrackerSchema) {
    return (
      db.categories.resolve(query) ??
      db.categories.find((c) => c.id.toLowerCase().includes(query)) ??
      db.categories.find((c) => c.name.toLowerCase().includes(query))
    );
  }
      ...
      builder
        ...
        .addSubcommandGroup((group) =>
          group
            .setName('create')
            .addSubcommand((sub) =>
              sub.setName('autorole').setDescription('Create a new autorole. Autoroles will respect the configured default donation category.')
            )
            .addSubcommand((sub) => sub.setName('category').setDescription('Create a new donation category.'))
        )
        .addSubcommandGroup((group) =>
          group
            .setName('delete')
            .addSubcommand((sub) => sub.setName('autorole').setDescription('Delete an existing autorole.'))
            .addSubcommand((sub) => sub.setName('category').setDescription('Delete an existing donation category.'))
        )
        .addSubcommandGroup((group) =>
          group
            .setName('weekly')
            .addSubcommand((sub) => sub.setName('leaderboard').setDescription('View the leading donators from a donation category this week.'))
            .addSubcommand((sub) => sub.setName('reset').setDescription('Reset the weekly donators leaderboard for the default donation category.'))
        )
    );
  }
}
