...
export class ChatInputCommandDeniedListener extends Listener<typeof Events.ChatInputCommandDenied> {
	...
	private get permissionReadables(): Record<Permissions, string> {
		return <Record<Permissions, string>>(
			Object.fromEntries(Object.keys(PermissionFlagsBits).map((key) => [key, toTitleCase(key.replaceAll('_', ' '))]))
		);
	}
	...
	public async run(error: UserError, payload: ChatInputCommandDeniedPayload) {
		...
		if (this.isClientMissingPermissions(error, error.context)) {
			...
			embed.setDescription(
				`You're in cooldown. You can use this command again ${time(
					new Date().getTime() + error.context.delay,
			...
			this.container.logger.error(
				`"${payload.interaction.user.tag} (${payload.interaction.user.id})" was blocked from using "${payload.context.commandName}" for an unknown reason`,
				{ error, payload }
			);
		}
	...
	public isClientMissingPermissions(error: UserError, context: unknown): context is Preconditions['ClientPermissions'] {
		return (
			Reflect.get(error, 'identifier') === Identifiers.PreconditionClientPermissions &&
	...
	public isCooldown(error: UserError, context: unknown): context is Preconditions['Cooldown'] {
		...
		return (
			Reflect.get(error, 'identifier') === Identifiers.PreconditionCooldown &&
			cooldownContextKeys.some((key) => Reflect.has(context as object, key))
		);
	}
	...
	public isMissingCommandHandler(error: UserError): boolean {
		return [
			Identifiers.PreconditionMissingChatInputHandler,
			Identifiers.PreconditionMissingContextMenuHandler,
			Identifiers.PreconditionMissingMessageHandler
		].includes(error.identifier as Identifiers);
	}
	...
	public isUserPermissions(error: UserError, context: unknown): context is Preconditions['UserPermissions'] {
		return (
			Reflect.get(error, 'identifier') === Identifiers.PreconditionUserPermissions &&
