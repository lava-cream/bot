import type { Awaitable, CollectorFilter, InteractionCollector, MappedInteractionTypes, MessageComponentType, User } from 'discord.js';
import type { TFunction } from '#lib/utilities/common/common.types';
import { isNullOrUndefined } from '@sapphire/utilities';
import { disableMessageComponents } from '#lib/utilities';
 ...
 */
export interface CollectorActionContext<in out T extends MessageComponentType, Cached extends boolean> {
	/**
	 * The action itself.
	 */
 ...
 * @template Cached The cached status of the interaction.
 */
export type CollectorActionLogic<T extends MessageComponentType, Cached extends boolean> = TFunction<
 ...
 */
export type CollectorFilterAction<T extends MessageComponentType, Cached extends boolean> = CollectorFilter<[MappedInteractionTypes<Cached>[T]]>;
 ...
 * @template Cached The cache status.
 */
export type CollectorEndAction<T extends MessageComponentType, Cached extends boolean> = (
	/**
	 * The context for this predicate.
	 */
	context: Omit<CollectorActionContext<T, Cached>, 'action' | 'interaction' | 'stop'> &
		Readonly<{
			/**
 ...
 */
export class CollectorAction<in out T extends MessageComponentType, Cached extends boolean> {
	/**
	 ...
	 */
	public static getDefaultFilterAction<T extends MessageComponentType, Cached extends boolean>(user?: User): CollectorFilterAction<T, Cached> {
	 ...
	 * @returns A {@link Function}.
	 */
	public static getDefaultEndAction<T extends MessageComponentType, Cached extends boolean>(): CollectorEndAction<T, Cached> {
		return async (context) => {
			if (isNullOrUndefined(context.interaction) || isNullOrUndefined(context.interaction.message)) return;

			await context.interaction.message.edit({ components: disableMessageComponents(context.interaction.message.components) });
		};
	}
}
