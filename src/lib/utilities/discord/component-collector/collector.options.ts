...
import type { Message, MessageCollectorOptionsParams, MessageComponentType, User } from 'discord.js';
 ...
 * @since 6.0.0
 */
export interface ICollectorOptions<T extends MessageComponentType, Cached extends boolean> extends MessageCollectorOptionsParams<T, Cached> {
 ...
 * @since 6.0.0
 */
export default class CollectorOptions<T extends MessageComponentType, Cached extends boolean>
	extends Builder
	implements ICollectorOptions<T, Cached>
{
	...
	public setActions(actions: ICollectorOptions<T, Cached>['actions']): this {
		if (actions) {
			this.collector.actions.set(Object.entries(actions).map((action) => this.collector.actions.construct(...action)));
		}
