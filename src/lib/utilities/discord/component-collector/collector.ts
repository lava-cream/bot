 ...
 * @since 4.7.2
 */
export class Collector<in out T extends MessageComponentType, Cached extends boolean = boolean> extends Builder {
	/**
	 * Actions Manager
	 * -
	...
	private async onCollect(
		interaction: MappedInteractionTypes<Cached>[T],
		collector: InteractionCollector<MappedInteractionTypes<Cached>[T]>
	): Promise<void> {
	 ...
	 */
	private async onEnd(
		collected: Collection<string, MappedInteractionTypes<Cached>[T]>,
		reason: string,
		collector: InteractionCollector<MappedInteractionTypes<Cached>[T]>
	): Promise<void> {
	 ...
	 */
	private static NativeCollectorEndReasons = [
		'time',
		'user',
		'idle',
		'limit',
