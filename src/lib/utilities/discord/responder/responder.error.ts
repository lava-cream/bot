...
import type { CacheType, BooleanCache, Message } from 'discord.js';
...
export namespace ResponderError {
	 ...
	 * @since 6.0.0
	 */
	export function send<Cached extends CacheType>(
		target: Responder.ResponderTarget<Cached>,
		message: string
	 ...
	 * @since 6.0.0
	 */
	export function edit<Cached extends CacheType>(
		target: Responder.ResponderTarget<Cached>,
		message: string
	): Promise<Message<BooleanCache<Cached>>> {
