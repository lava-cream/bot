...
import { CustomId, join, MessageContentBuilder, type StringSelectMenuComponentBuilder } from '#lib/utilities';
import { bold } from '@discordjs/builders';
import { Resolvers } from '@sapphire/framework';
import { isNullOrUndefined } from '@sapphire/utilities';
import type { CommandInteraction, MessageEditOptions, MessageCreateOptions } from 'discord.js';
...
export class DonationDeskManager extends Manager<DonationDeskSchema> {
	...
	public async sendOrUpdateMessage(db: DonationDeskSchema.Document, command: CommandInteraction<'cached'>): Promise<boolean> {
		...
		const messageContent = new MessageContentBuilder<StringSelectMenuComponentBuilder>()
		...
		try {
			if (srcMessage.isOk()) {
				...
				await channel.unwrap().send(messageContent as MessageCreateOptions);
			}
